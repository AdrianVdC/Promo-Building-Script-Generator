# NATS Bonus Creator — Technical Reference v2.17

## Changes from v2.16

- **Lifecycle - Churn DM** — ninth offer type fully integrated into the HTML tool. Deposit-match offer paid in **Casino Credit** for churn-risk customers. Builds Segment, Promotion, and Bonus for each selected offer on each day card. Offer rows are **Min → % → Max** with a per-offer checkbox; each offer must map to a baked-in Terms ID or **script generation is blocked** with a warning. Generates `lc_churn_dm_MMDDYY_HHMM.py`. Validated field-by-field against NATS via staged test scripts before integration (including the Casino Credit-specific "Casino Credit (%)" checkbox label discovered during testing).

---

## HTML Tool Layout

- **Left column (340px fixed):** Branding, primary action buttons, Offer Type selector, Region selector (hidden for RTC FS, RAF, SUO, and BGRE)
...
**Edit HIW / Edit T&Cs**
Hidden for RAF, SUO, and BG-RE (copy pending legal approval or not applicable). Active for RTC CC, RTC FS, BG, DM, LC-REACT, and LC-CHURN-DM.

**Edit Images**
Present for RTC CC, RTC FS, RAF, LC-REACT, and LC-CHURN-DM. Hidden for BG, DM, SUO, and BG-RE.

**+ Add Day**
No-op for RAF and SUO. Max 8 days for BG/DM, 99 for RTC CC/FS/LC-REACT/LC-CHURN-DM.

**Clear Saved Settings**
Clears all localStorage keys including `lc_react_image_path` and the `lc_churn_dm_*` keys.
...
12. Amount = parsed from code name
...
15. Bonus amount summary card — tries `Bonus` then `Casino Credit` card title; fill with `{max}` from code
16. Minimum Deposit = **dynamic** `{min}` from code (`M{min}` segment)
17. Maximum Deposit = `1000000` (hardcoded)
...
Poll Create Bonus → Opt In → name/description → Refer A Friend checked → Referee radio → Send To XP NOT clicked → remove platform tags → Voucher Code → Free Spin type → description/CTA/game fields → per-day spins → spin value → Days (0/0/1) → Status Active checked → Activation → Reporting fields → Segment → Promotion Tile → Save → voucher modal → Apply
