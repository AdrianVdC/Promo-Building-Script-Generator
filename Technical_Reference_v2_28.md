# NATS Bonus Creator — Technical Reference v2.28
...
Date (MMDDYY) with a live **weekday pill** rendered beside the input as soon as a valid date is typed — green `{Weekday} • Casino Credit` for Tue/Thu/Sat, amber `{Weekday} • Free Spins — {game}` for Mon/Wed/Fri/Sun. Step badges: **Promo + Bonus only** (BG-RE style — the segment is attached inside Playmaker by code name). Region row hidden (fixed CA/Ontario); Edit HIW / Edit T&Cs buttons hidden (copy baked in); Edit Images visible.
...
**FS-day state (Mon/Wed/Fri/Sun):** amber lock notice ("Free Spins days are not yet supported — Playmaker Bonus Spins setup is pending"); the four FS tiers render greyed and disabled with that weekday's Terms IDs visible ($50/20 spins @ $0.10, $125/50 @ $0.10, $500/50 @ $0.50, $5,000/100 @ $3.00; Mon `CAS_CA_0043`–`0046`, Wed `0047`–`0050`, Fri `0051`–`0054`, Sun `0055`–`0058`); step badges auto-reflect nothing generatable and `generateScript` blocks with an alert if an FS-day date is present.

Weekday→game map (`DMCA_GAMES`, used by the pill now and reserved for the FS unlock): Monday = Baa Baa Baa™ (BAA), Wednesday = 4 Crazy Cluckers™ (4CC), Friday = Mega Fire Blaze™: Legacy of the Tiger™ (MFB), Sunday = Blue Wizard: Cash Collect & Link™ (BWZ). Aggregator/Provider/Casino Game values for Playmaker FS setup are not yet configured (to be supplied later).
...
BG-RE and DMCA show **Promo and Bonus** badges only — no Segment badge. The segment is created/attached inside Playmaker as part of the bonus flow.
...
|---|---|---|
| Reward | Casino Credit (CAD row) | FanCash (USD row) |
| Code name | `MMDDYY_VCL_RET_DM_CA_CC_M{min}_{pct}M{max}` | `MMDDYY_VCL_RET_DM_US_FC_M{min}_{pct}M{max}` |
...
## Script Steps — Daily Missions - Canada (v2.28)

Two-phase production script (`daily_missions_MMDDYY_HHMM.py`). `CODES` entries carry `code_name`, `weekday`, `bet`, `get`, `terms_id`, `days_to_fulfill`, `do_promo`, `do_bonus`. Phases are skipped entirely if no code has that step enabled.
...
### Phase 1 — NATS Promotion (per code with `do_promo`)
1. Manual login pause (`input()`), then sidebar hover-nav to Promos (`[data-menu-id$='-cms']`, 5-attempt retry with safe-zone reset)
...
6. Images (8s post-select wait, 2s post-OK — standard timing): `{Weekday}_Promo_Detail.png` → Promo Detail, `{Weekday}_Masthead.png` → Masthead, `{Weekday}_B{bet}_G{get}.png` → Discover (weekday derived at runtime from the code's date)
7. Title and Promo Header Text = `{Weekday}'s Daily Mission` (auto-derived)
8. **Bonus Tile switch ON**; **Button 2 left empty** (by design)
9. How it works = `HIW_CC_TEMPLATE.format(bet_fmt, get_fmt)` — no "CAD" anywhere in HIW by design; "twenty-four (24) hours" is static legal copy
10. T&C = `TC_CC_TEMPLATE.format(bet_fmt, get_fmt, terms_id, start_date_long, end_date_long)` — full Ontario legal copy; `TERMS_START_LONG` / `TERMS_END_LONG` baked at generation time (currently `August 17, 2026` / `December 31, 2026`); §5's $2/$20/$5 worked examples and §4/§7's "each time you are eligible" clauses are static legal copy on all tiers
...
3. **Step 1 Details:** name = code name; jurisdictions = uncheck `#jurisdiction-all` if checked, check `#jurisdiction-ontario` (Canada); Start Date = code date, End Date = code date + 1 day via the calendar widget (`button.dc-w-\[280px\]` pickers, `aria-live` month header, `button[name='next-month']`, `button[name='day']` skipping `dc-day-outside`); both times set to `00:00` via the native-setter `evaluate` pattern (React-controlled inputs)
4. **Step 2 Eligibility:** segment chip — fill `input[cmdk-input]` with the code name + Enter (chip attach confirmed in PROD)
5. **Step 3 Entitlement:** `input[name='wageringAmount']` = bet
6. **Step 4 Settlement:** **Casino Credit radio** (`button[role='radio'][value='casino_credit']`, clicked only if `aria-checked` ≠ true); `input[name='settlementAmount']` = get; `input[name='daysToFulfill']` = Days to Use Reward (cleared and refilled only if different)
7. **Step 5 Display:** promotion combobox (`button[role='combobox']` → search input → option clicked via `get_by_label(name).get_by_text(name)`) selecting the Phase 1 promo by code name; **toast always ON** — `#toastEnabled` checked if not already, `input[name='toastTitle']` = `Promotion Complete!`, `input[name='toastDescription']` = `You have been awarded ${get} Casino Credit` (FS variant `You have been awarded {spins} Bonus Spins to "{game_name}"` recorded for the unlock)
8. **Step 6 Review:** `button:has-text('Save Bonus')` clicked, 3s settle
9. macOS `osascript` notification at the end; final `input()` pause before the persistent context closes

⚠️ **FS days (Mon/Wed/Fri/Sun) are excluded at generation time** — the tool blocks them; the generated script never contains an FS code. The Playmaker Free Spins spec (Bonus Spins radio, Number of Spins, Aggregator/Provider/Casino Game per weekday game, Bet Level default, Free Spin Stakes = spin value, Days to Use Reward 7) is recorded but not implemented — Playmaker Bonus Spins is not yet functional, Aggregator/Provider/Game values are not yet known, and the observed Free Spin Stakes row showed USD (CAD appearance for Canada unconfirmed).
...
| 11 | DMCA | **Free Spins days (Mon/Wed/Fri/Sun) are locked** — Playmaker Bonus Spins setup is not yet functional. Only Casino Credit days (Tue/Thu/Sat) can be generated. Unlocking requires: Playmaker Bonus Spins working, Aggregator/Provider/Casino Game values per weekday game (not yet known), confirmation that the Free Spin Stakes row offers CAD for Canada (observed USD), FS toast copy wiring, and wiring the recorded FS T&C template into the builder. FS tiers, Terms IDs, and the weekday→game map are already configured in the HTML. |
| 12 | DMCA | HIW, T&C, Terms IDs, and promotional dates are baked into the HTML at generation time with **no edit modals** (Edit HIW / Edit T&Cs hidden by design) — changes require editing the HTML (reach out to Adrian) and regenerating scripts. The T&C was byte-verified against the supplied CAS_CA_0019/0020 filings (with two approved normalizations); **CAS_CA_0021/0022 and all FS-day filings were not supplied** and are assumed to follow the templates — verify on first use. |
| 13 | DMCA | CC code names are shape-identical to standard BG CA codes (`MMDDYY_CAS_RET_BG_CA_CC_B{bet}_G{get}`) — accepted by stakeholder decision; no distinguishing token. Avoid building a standard BG CA offer with the same date/bet/get, and note the Aug 14 2026 validation promo `120126_CAS_RET_BG_CA_CC_B125_G5` already exists in PROD NATS (delete or avoid reusing that exact code). |
