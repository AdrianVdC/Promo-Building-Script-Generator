...
## Daily Missions - Canada (v2.28; FS unlocked v2.30; NATS segments v2.31; toast title v2.34)
...
Both: Days to Use Reward from the card, promotion selected by code name, **toast always ON** (**`Mission Complete!` as of v2.34** / CC: `You have been awarded $X Casino Credit` / FS: `You have been awarded {spins} Bonus Spins to {game_name}` — **no quotes around the game name**), then **Save Bonus**. Each bonus runs in a per-bonus try/except with recovery navigation back to Bonus Setup on failure, and a **SAVED/FAILED results summary** prints at the end of Phase 2.
...
Fully integrated into the HTML tool as of v2.15. See `Technical_Reference_v2_34.md` for full selector reference and Playmaker field spec.
...
### DMCA Toast (v2.34)
The DMCA Playmaker toast is always ON. Title: **`Mission Complete!`** (changed from `Promotion Complete!` in v2.34). Description: CC `You have been awarded $X Casino Credit`; FS `You have been awarded {spins} Bonus Spins to {game_name}` (no quotes around the game name). BG-RE's toast is separate and still uses `Promotion Complete!`.
...
**Toast (v2.34):** always ON — title `Mission Complete!`; descriptions unchanged (see the DMCA section above). Baked into the HTML like the rest of the DMCA copy.
