...
## Daily Missions - Canada (v2.28; FS unlocked v2.30)
...
Steps 1–6: **Canada** jurisdiction (`#jurisdiction-ontario`), start/end dates via the calendar widget with both times forced to **00:01**, **segment = code name** typed into the "Create Account Segments" field + Enter — ⚠️ the chip **creates the segment when Save Bonus is clicked** (any new name works; no pre-existing segment is needed — earlier "chip attaches existing segment" wording was corrected in the v2.30 doc pass), wagering amount = bet, then **Step 4 branches on reward**:
...
### PROD Validation
- **Aug 14, 2026 (v2.28):** NATS promo `120126_CAS_RET_BG_CA_CC_B125_G5` built live end-to-end; Playmaker CC dry run through Step 6. ⚠️ **This test promo still exists in PROD NATS** — delete it or avoid reusing that exact code.
- **Aug 14–19, 2026 (v2.30 FS work):** iterative dry runs captured the FS selectors and all four stakes ladders; **full FS flows built live with Save Bonus clicked** — `120426` MFB (two tiers), `121626` 4CC, and a live same-day `081926` 4CC $50 build confirmed in-app. The v2.30 builder's rendered FS T&C is byte-identical to the PROD-validated test-script template. ⚠️ **All FS test builds (and their auto-created segments) should be deleted from PROD** along with the `120126` CC promo.
...
| 16 | DMCA | CC code names are shape-identical to standard BG CA codes (`MMDDYY_CAS_RET_BG_CA_CC_B{bet}_G{get}`) — accepted by stakeholder decision, no distinguishing token. Avoid building a standard BG CA offer with the same date/bet/get. ⚠️ **PROD cleanup pending:** the Aug 14 validation promo `120126_CAS_RET_BG_CA_CC_B125_G5` and the v2.30 FS validation builds (`120426` MFB ×2, `121626` 4CC, `081926` 4CC — promos, bonuses, and their auto-created segments) still exist in PROD; delete them or avoid reusing those exact codes. |
| 17 | DMCA | The Okta Verify (FastPass) persistent-profile launch (`~/.dm_canada_pw_profile` + `--disable-features=LocalNetworkAccessChecks` + `local-network-access` grant) is DMCA-only; all other offer types still use plain `chromium.launch()`. Back-porting is a candidate follow-up if Okta blocks appear elsewhere. The v2.30-generated script itself has not had a live run, but the identical FS and CC flows were PROD-validated Aug 14–19 via test scripts whose T&C output the builder matches byte-for-byte — treat the first v2.30 generation run as routine confirmation. |
...
**Promo** and **Bonus** badges only — no Segment badge. The segment is created inside Playmaker as part of the bonus flow (DMCA types the code name into the "Create Account Segments" field in Step 2 — the chip creates the segment on Save Bonus).
...
Fully integrated into the HTML tool as of v2.15. See `Technical_Reference_v2_30.md` for full selector reference and Playmaker field spec.
...
LC-REACT, LC-CHURN-DM, VIPDM, and VIPBG use the same anchored regex pattern as the Promotion Tile — scoped to `.ant-select-dropdown:not(.ant-select-dropdown-hidden)` — to avoid hitting the hidden search mirror span that NATS injects alongside the input. DMCA has no NATS segment — the code name is typed into Playmaker's "Create Account Segments" field in Step 2 (fill `input[cmdk-input]` + Enter); ⚠️ the chip **creates the segment when Save Bonus is clicked** — any new name works, no pre-existing segment is needed (v2.30 doc correction; earlier docs said the chip attaches an existing segment).
...
3) We'll reward you with {get_fmt} in Casino Credit to use on any game on Fanatics Casino
```
**FS HIW (v2.30 — live in the builder):** line 3 becomes `We'll reward you with {spins} Bonus Spins to use on the slots game {game_name}`.
