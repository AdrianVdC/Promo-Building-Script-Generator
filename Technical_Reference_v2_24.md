# NATS Bonus Creator — Technical Reference v2.24
...
- **Left column (340px fixed):** Branding, primary action buttons, Offer Type selector, Region selector (hidden for RTC FS, RAF, SUO, and BGRE; restricted to US / CA for RTC CC as of v2.22; locked to CA — Canada for VIPDM as of v2.23 and VIPBG as of v2.24)
...
Single card (full width, max 680px). Title: **VIP Offer Library — Bet & Gets — CA — Canada**. `+ Add Day` is a no-op.
Date (MMDDYY, free-typed — same month-anchored window rules as VIPDM), offers list (**Category → Bet → Get** rows with a per-row Slots/Tables dropdown and per-offer checkbox — eight defaults shown unselected: Slots $1,000/$100, $5,000/$500, $20,000/$1,000, $100,000/$5,000; Tables $2,000/$100, $10,000/$500, $25,000/$1,000, $100,000/$2,500 — each row displays its Terms ID inline, red `no ID` when unmapped), + Add / Select All, step badges (Segment/Promo/Bonus), Days to Opt In (3) / Days to Entitlement (3 — **extends the promo/bonus end date**) / Days to Settlement (7). **Send to XP and External Bonus have no toggles — both are always ON** (static note on the card). Region selector locked to **CA — Canada**. No Campaign Name field and no ZIP attachment — Title, Promo Header, and Bonus Description are fixed as `From Your VIP Host`, and images come from a locally-synced folder.

**Terms ID enforcement:** identical to VIPDM — each active offer key `{SBG|TBG}_B{bet}_G{get}` must exist in the VIPBG Terms ID map (default `CAS_CA_0069`–`CAS_CA_0076`, editable via the Edit T&Cs modal's Terms bar). Active offers without an ID show a red row warning and block script generation with an alert. Keying includes the category prefix as collision insurance (a Slots and Tables tier could otherwise share a `B{bet}_G{get}` shape).
...
## Script Steps — VIP Offer Library - Bet & Gets (v2.24)

CA — Canada only. Every code carries the `CA` token, so all currency-table fills land in the **CAD** row via the v2.18 helpers. **The wager requirement ("bet") is enforced upstream by XP / segment logic — it is never entered on the bonus.** The bonus is a flat Casino Credit amount equal to the "get" value.
...
### `parse_vipbg(code)`
Returns `(cat, bet, get)` from `MMDDYY_VCL_RET_{SBG|TBG}_CA_CC_B{bet}_G{get}` — the category token is matched against the literal set `("SBG", "TBG")`; bet/get from `B`/`G`-prefixed all-digit tokens (so `VCL`, `RET`, `CA`, `CC` are skipped).

### `GAME_CATEGORIES` map
`SBG` → HIW `Slots`, T&C `slots` / `SLOTS`; `TBG` → HIW `Tables`, T&C `table` / `TABLES`. The lowercase asymmetry ("slots games" but "table games") matches the legal documents verbatim.

### `create_segment(page, name)`
Identical to RTC CC / DM / Churn DM / VIPDM — AMELCO `dispatch_event("click")`, 2-second post-`#forBonus` wait.

### `create_promo(page, name, idx)`
Poll Create Promotion (120 × 500ms, up to 60s) → click → fill name → Start/End per `parse_dates(name, DAYS_TO_ENTITLEMENT[idx])` → Type: `Image only CTA` → Layout: `Overlay` → first Save → upload `Promo Detail.png`, `Masthead.png`, `{SBG|TBG}_B{bet}_G{get}.png` from `IMAGE_FOLDER` → Promo Header Text: `From Your VIP Host` → Title: `From Your VIP Host` → Bonus Tile toggle ON → **Button 2 label/link intentionally left EMPTY** (CA — Canada; the terminal prints `-- Button 2: left empty`; `Play Now!` → `/docs/usered/casgenericgamelist` is documented in the script header, reserved for the future US region) → How it works (template with `{bet_fmt}` / `{get_fmt}` / `{game_category}`) → T&C (single Canadian legal template covering both categories, baked in at generation time) → second Save. No confirmation modal.
...
5. Remove **all three** COMBO platform tags — `COMBO: Sportsbook`, `COMBO: Sportsbook And Casino`, and `COMBO: Casino` — leaving **`Web` + `STAC: Standalone Casino` only** (matching VIPDM)
6. Voucher Code: `CLBGFC{MMDDYY}{XXX}`
7. Bonus type: `Casino Credit` (flat amount — no percentage checkbox, no deposit entitlement, no Matched Deposit)
8. Days to Meet Casino_credit Settlement = per-card value (default 7)
9. Days to Opt In = per-card value (default 3)
...
12. Casino Credit amount = **`{get}` from the code** via `fill_currency_amount` — **CAD row**
13. Status Active = checked
...
21. Save — **defensive confirmation-modal handling**: waits up to 5s for `.ant-modal-content`; confirms it if it appears, continues with a terminal note if it doesn't. Production runs confirmed no modal appears on this form, but the branch stays as insurance since standard BG (FanCash) does show one.
...
| Trigger | Opt In |
| Description | From Your VIP Host |
| Send To XP / External Bonus | Both always ON |
| Bet amount | **Not entered anywhere** — enforced upstream (XP/segment) |
| Casino Credit amount | From code (`G{get}`) — CAD row |
| Stake Chunk Sizes | 1 (conditional fill) |
| Platforms | Web + STAC: Standalone Casino only |
| Status Active | Checked |
| Reporting Platform | CAS (Casino) |
| Bonus Origin | Opt-In Bonus |
| Series | VIP |
| Type | Retention |
| Confirmation modal | None observed (handled defensively) |
...
### VIPBG T&C Data (Baked In at Generation Time)
...
**VIP Offer Library exception (v2.23/v2.24):** VIPDM and VIPBG both remove **all three** COMBO tags — `COMBO: Sportsbook`, `COMBO: Sportsbook And Casino`, and `COMBO: Casino` — leaving only `Web` + `STAC: Standalone Casino`. They are the only offer types that remove `COMBO: Casino`.
...
| 8 | VIPBG | HIW, T&C, Terms IDs, and promotional dates are baked in at generation time — changes via the Edit T&Cs / Terms modal require regenerating the script. New offer sizes require a Terms ID (editable in the Terms modal, keyed `{SBG\|TBG}_B{bet}_G{get}`); active offers without one block script generation by design. |
| 9 | VIPBG | CA — Canada only. Button 2 is intentionally left empty for CA; `Play Now!` → `/docs/usered/casgenericgamelist` is reserved for the planned US region (values documented in the generated script header). Series `VIP` shares limitation #6 with VIPDM. |
