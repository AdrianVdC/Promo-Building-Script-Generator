# NATS Bonus Creator — Technical Reference v2.30
...
## Script Steps — Daily Missions - Canada (v2.28; FS unlocked v2.30)

Two-phase production script (`daily_missions_MMDDYY_HHMM.py`). `CODES` entries carry `code_name`, `reward` (`CC` / `FS`), `weekday`, `bet`, `get`, `spins`, `spin_value`, `vtok`, `game_name`, `game_search`, `terms_id`, `days_to_fulfill`, `do_promo`, `do_bonus` (FS-only fields are empty strings on CC codes and vice versa for `get`). Phases are skipped entirely if no code has that step enabled.
...
4. **Step 2 Eligibility:** segment — fill `input[cmdk-input]` with the code name + Enter. ⚠️ This is a **"Create Account Segments"** field: the chip **creates** the segment when Save Bonus is clicked (works for any new name; no pre-existing segment is needed). Earlier docs described this as attaching an existing segment — corrected in the v2.30 pass.
