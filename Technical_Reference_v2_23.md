# NATS Bonus Creator — Technical Reference v2.23
...
- **Left column (340px fixed):** Branding, primary action buttons, Offer Type selector, Region selector (hidden for RTC FS, RAF, SUO, and BGRE; restricted to US / CA for RTC CC as of v2.22; locked to CA — Canada for VIPDM as of v2.23)
...
**Edit HIW / Edit T&Cs**
Hidden for RAF, SUO, and BG-RE (copy pending legal approval or not applicable). Active for RTC CC, RTC FS, BG, DM, LC-REACT, LC-CHURN-DM, and VIPDM. For RTC CC, Edit T&Cs is **jurisdiction-aware** (v2.22): it reads and saves the selected region's T&C (`rtc_tc` vs `rtc_tc_ca`) and displays the region in the modal title. Edit HIW is shared across US and CA (single `rtc_hiw` key) by design. For **VIPDM** (v2.23), the Edit T&Cs modal shows the Terms bar with an **Edit Terms Expiry & Terms IDs** button routed to the VIPDM terms modal — promotional start/end dates (`{start_date_long}` / `{end_date_long}`) plus one Terms ID input per offer key `M{min}_{pct}M{max}`.

**Edit Images**
Present for RTC CC, RTC FS, RAF, LC-REACT, LC-CHURN-DM, and VIPDM. Hidden for BG, DM, SUO, and BG-RE. For RTC CC, jurisdiction-aware (v2.22): reads/saves `rtc_image_path` vs `rtc_image_path_ca`, shows the region in the modal title, and the Open Shared Images Folder link switches between the US and Canadian Google Drive folders.

**+ Add Day**
No-op for RAF, SUO, and VIPDM (single card each). Max 8 days for BG/DM, 99 for RTC CC/FS/LC-REACT/LC-CHURN-DM.

**Clear Saved Settings**
Clears all localStorage keys including `lc_react_image_path`, the `lc_churn_dm_*` keys, all RTC CC `_ca` keys (v2.22), and all `vcl_dm_*` keys (v2.23).
...
## Per-Code Step Toggles (RTC CC / FS / BG / DM / LC-REACT / LC-CHURN-DM / VIPDM)
...
### RTC CC / FS / BG / DM / LC-REACT / LC-CHURN-DM / VIPDM
...
Applies to all eight pre-v2.23 NATS offer types (RTC CC, RTC FS, BG, DM, RAF, SUO, LC-REACT, LC-CHURN-DM). BG-RE bonuses are built in Playmaker, not NATS, so platform tags do not apply.

**VIPDM exception (v2.23):** VIP Offer Library - Deposit Matches removes **all three** COMBO tags — `COMBO: Sportsbook`, `COMBO: Sportsbook And Casino`, and `COMBO: Casino` — leaving only `Web` + `STAC: Standalone Casino`. It is the only offer type that removes `COMBO: Casino`.
...
### Shared helpers (embedded in all ten generated script templates)
...
- Google Drive for Desktop required for RTC CC, RTC FS, RAF, LC-REACT, LC-CHURN-DM, and VIPDM image path access
