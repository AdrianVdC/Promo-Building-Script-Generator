# NATS Bonus Creator — Technical Reference v2.27
...
The base `vcl_bg_*` and `vcl_dm_*` keys hold the CA — Canada values (v2.25 / v2.26). SUO Day 2+ Spins and BG-RE have no localStorage keys. Clear Saved Settings removes all keys above, including every `_ca` and `_us` variant.
