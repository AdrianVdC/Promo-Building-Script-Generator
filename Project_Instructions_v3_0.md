# NATS Bonus Creator — Project Instructions (v3.0)
...
**Current live version:** `nats_bonus_creator_v3_0.html`

> ⚠️ **All scripts should be generated from v3.0.** v3.0 introduced the new naming convention for every live offer type, the 40-character NATS name guard, positional fail-loud code parsing, and the Promotion Tile lag retry. Every live offer type was PROD-validated on v3.0 (Aug 25, 2026). The long v2.x version-gating rules ("Canadian RTC CC from v2.22+ only", "DMCA from v2.31+ only", etc.) are obsolete for generation purposes — they are preserved in `Technical_Reference_v3_0.md` § Version History for anyone auditing old scripts. Voucher codes, image filenames, and Terms ID localStorage maps are **unchanged from v2.34**.
...
DMCA runs both phases in **production** (NATS + Playmaker) and is the only offer type that builds bonuses in Playmaker. Full selector reference and field spec are in `Technical_Reference_v3_0.md`.
...
| MECHANIC | `OPT` Opt-in Bonus · `BG` Bet and Get · `DM` Deposit Match · `DR` Drop · `SBG` Slots Bet & Get · `TBG` Tables Bet & Get · `RAF` Refer a Friend | `LOSS` Lossback · `SWP` Sweepstakes · `GW` Goodwill · `S2W` Spin to Win Bulk Uploads · `LB` Leaderboard · `FCX` FanCash Multiplier |
...
Notes:
- **VIPBG:** the SBG/TBG category lives in the MECHANIC slot (`parts[5]`), no longer in the freeform. US always carries `SBG` (all US tiers are slots). RAF's whole campaign carries mechanic `RAF` (Day 2+ payouts included); SUO carries `DR` (Drop).
...
- **Terms ID keys are unchanged:** the localStorage maps keep their v2.x literal key shapes (`M{min}_{pct}M{max}`, `{SBG|TBG}_B{bet}_G{get}`, `B{bet}_G{get}`); scripts rebuild them from K-expanded parse results, so saved maps survive the v3.0 upgrade with no reset.
...
- VIPDM discover (both regions): `{pct}M{max}.png` — ⚠️ several names differ by one character: CA `10M250.png` vs `100M250.png`; US `10M500.png` vs `10M5000.png` and `20M500.png` vs `20M5000.png`; double-check those creatives
...
- LC-REACT / LC-CHURN-DM: US (default), MI, WV, PA, NJ, ON, AB, CA
- RAF / SUO: MI, NJ, WV, PA only
...
Full field specs, selectors, T&C templates, and window math are in `Technical_Reference_v3_0.md`. What follows are the operational essentials per offer type — all behavior below is **unchanged from v2.34** except code names and the retry.
...
Single card; **always defaults to US on offer-type selection**. US: FanCash/USD, ten tiers `CAS_10677`–`10686`, dates 07/01/26 → 12/31/26, `Bonus (%)` checkbox, `Days to Meet Fancash Settlement`, COMBO: Casino kept, FanCash confirmation modal expected (handled defensively). CA: Casino Credit/CAD, nine tiers `CAS_CA_0078`–`0086`, dates 08/17/26 → 12/31/26, `Casino Credit (%)`, all COMBO tags removed, no modal. Both: **month-anchored window** (typed date − 1 day 00:00 ET → month-end 23:59:59 ET + Days to Entitlement), `From Your VIP Host` copy, Button 2 `Deposit` → `/auth/account/deposit`, Matched Deposit always checked, Series `VIP` / Type `Retention` (visible-dropdown scoped — load-bearing), Origin Opt-In Bonus. All modals region-aware; region switch silently resets. ⚠️ CAS_10679–10685 filings unsupplied — verify the canonical template on first use of those tiers.
...
The full key table is identical to v2.34 — `rtc_*` (+ `_ca`), `rtc_fs_*`, `bg_*`, `dm_*`, `raf_image_path`, `lc_react_*`, `lc_churn_dm_*`, `vcl_dm_*` (+ `_us`), `vcl_bg_*` (+ `_us`), `daily_missions_image_path`. v3.0 added no keys and changed no key shapes (Terms ID map keys keep their literal v2.x forms). Retired offer types' keys remain functional but unused while retired. Clear Saved Settings removes everything, including every `_ca` and `_us` variant. SUO, BG-RE, and DMCA (image path aside) have no localStorage keys.

---

## Image System (UNCHANGED from v2.34)

All paths, Drive folders, and filename conventions are identical to v2.34 — see that section of `Technical_Reference_v3_0.md` for the full folder list. The operational rule stands:
...
| 2 | VIPDM / VIPBG | Exact-40 codes exist (`M250_10M2500`, `TBG…B100K_G2500`); future tier additions must re-check the budget — the generation-time guard blocks silent overflow. HIW/T&C/Terms IDs/dates remain baked at generation time; new tiers need Terms IDs (region-scoped) or generation blocks. |
...
| 5 | Template literals | Unchanged and still critical: generated Python lives inside JS template literals which swallow single backslashes; **any change must be verified by rendering through a real JS engine and executing the resulting Python** (the v3.0 migration followed this for every increment — see `Changelog_v3_0.md`; note the nested-literal variant inside `${cond ? \`…\` : \`…\`}` blocks has different backslash arithmetic). |
...
All HIW and T&C templates, dynamic fields, title lines, and byte-verification notes are **unchanged from v2.34** — see `Technical_Reference_v3_0.md` for the full copy documentation per offer type. Summary of what's baked vs editable:

- **Editable via modals (region-aware where dual):** RTC CC HIW (shared) + T&C (per region); LC-REACT HIW/T&C; LC-CHURN-DM HIW/T&C; VIPDM HIW/T&C/Terms/dates (per region); VIPBG HIW/T&C/Terms/dates (per region).
- **Baked, no modals:** DMCA (all copy; changes via Adrian). RAF and SUO have placeholder/no copy respectively.
...
- All four builders are **byte-identical to v2.34** in the shipped v3.0 HTML (verified against the pristine file) — revival = re-add the `<option>` + run a v3.0-style migration increment.

---

## Related Documents

- `nats_bonus_creator_v3_0.html` — the tool (source of truth)
- `Technical_Reference_v3_0.md` — full selector reference, field specs, per-offer Script Steps, version history v2.17 → v3.0
- `Changelog_v3_0.md` — the v3.0 release record (format tables, verification record, harness catches, coexistence notes)
- `v3_0_naming_convention_spec_v0_1.md` — the design-of-record for the naming convention (decision log included)
