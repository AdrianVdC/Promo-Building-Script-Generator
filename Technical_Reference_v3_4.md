# NATS Bonus Creator — Technical Reference v3.4

## Changes from v3.2 (v3.3 / v3.4)

- **v3.3/v3.4 — Three new VIPDM CA tiers.** `DEFAULT_VIPDM_OFFERS` (CA) grows from nine to twelve: v3.3 added $250/20%/$10,000 (`CAS_CA_0092`, code freeform `M250_20M10K`, 39 chars) and $250/20%/$20,000 (`CAS_CA_0093`, `M250_20M20K`, 39 chars); v3.4 added $100/50%/$5,000 (`CAS_CA_0094`, `M100_50M5K`, 38 chars — listed with the other M100 tiers). All three filings match the baked CA Ontario template byte-for-byte (no template change); Terms ID keys keep their literal expanded shapes (`M250_20M10000`, `M250_20M20000`, `M100_50M5000`) and the existing `parse_vipdm` K-expansion handles the codes with no parser change. New Discover creatives: `20M10000.png`, `20M20000.png`, `50M5000.png` — ⚠️ new CA one-character near-collisions `20M1000.png` vs `20M10000.png` and `20M2000.png` vs `20M20000.png`.
- **v3.3 — Per-tier Terms start-date overrides (VIPDM, CA only).** The CAS_CA_0092/0093/0094 filings begin **September 1, 2026** while the shared CA start date remains 08/17/26 (filings 0078–0086). A `VIPDM_TERMS_START_OVERRIDES` constant (`{key: 'MM/DD/YY'}`; empty for US) is baked into the generated script as `TERMS_START_OVERRIDES = {key: [short, long]}`; `format_hiw`/`format_tc` look the tier key up there before falling back to `TERMS_START_SHORT/LONG`, so mixed-tier runs type the correct Promotional Period per tier. The Edit T&Cs modal's start-date field continues to govern non-overridden tiers only and carries a warning naming the overridden tiers. The end date (12/31/26) is shared by all twelve filings.
- **v3.3 — `getVIPDMTermsIDs()` default merge.** Baked defaults are now merged **under** any saved `vcl_dm_terms_ids` map (saved values win per key), so machines with a pre-v3.3 saved map resolve the new tiers' IDs immediately instead of blocking generation. Pre-v3.3 saved maps cannot contain the new keys, so no saved value is ever overridden. Applies to both regions through `vipdmKey()`.
- **Verification (v3.3/v3.4):** only the VIPDM surfaces changed (defaults, Terms map, override map, `getVIPDMTermsIDs`, `buildVIPDMScript`, info box, Terms-modal hint, version strings); all other builders untouched, retired builders byte-identical to v2.34. Per the template-literal limitation: full `<script>` passes `node --check`; `buildVIPDMScript` rendered through node v22 and executed — a three-code CA run byte-verified all three new filings' title/period/reward lines, the shared-date control (`M100_50M250` → August 17 + CAS_CA_0078) and the v3.3 tiers regression-clean, US render clean (`TERMS_START_OVERRIDES = {}`, July 1, 2026 intact), all codes ≤40, and the Terms merge tested against both a fresh machine and a simulated pre-v3.3 saved map.

---

## Changes from v3.1 (v3.2)

- **v3.2 — SUO mechanic swapped `DR` → `BG`.** SUO Day 2+ codes now read `MMDDYY_US_CAS_ACQ_SUO_BG_FS_DAY{n}_{spins}_{ST}` (e.g. `090926_US_CAS_ACQ_SUO_BG_FS_DAY3_50_WV`). Same token length — no budget change (DAY10 worst case stays 39). No collision: SUO's `ACQ`+`SUO` prefix distinguishes it from DMCA/retired-BG's `RET`+`BAU`, and RAF differs at the mechanic slot. `DR` (Drop) returns to the reserved/unused vocabulary. A stakeholder request to remove the mechanic section entirely for SUO was considered and declined in favor of this swap — it would have broken the universal 7-fixed-field parse contract. Two construction sites changed (SUO's Python `day_name` and the JS guard); only `buildSUOScript` differs from v3.1 (plus the version docstring in `buildDMCAScript`); rendered + executed verification per the template-literal limitation.

---

## Changes from v3.0 (v3.1)

- **v3.1 — Jurisdiction group + state suffix.** Position 1 of every code is always the two-character group `US` (US/MI/WV/PA/NJ) or `CA` (CA/ON/AB); a specific state/province rides at the **end of the freeform** (Build WV → Correct `090926_US_CAS_ACQ_SUO_DR_FS_DAY3_50_WV`; `DR` became `BG` in v3.2). Applies to all offer types; in practice only RAF, SUO (state-only per-card selects — suffix always present), and LC-REACT (suffix when a state/province is picked) produce suffixed codes, since RTC CC / VIPDM / VIPBG / DMCA / LC-CHURN-DM only offer US/CA. LC-REACT's `parse_amount` strips an optional trailing state token (set `{MI,WV,PA,NJ,ON,AB}`) before validation; RAF/SUO parse nothing. **RAF/SUO internals keep the real state** — vouchers (`WVAQ…`), Casino Game search (`Hotstepper (WV)`), and spin-stakes currency all still derive from the `JURISDICTION` constant; only the code names carry the group + suffix. Shared JS helpers `jurGroup()` / `jurSuffix()` live in `generateScript`. New exact-40 case: RAF `DAY10_{2-digit spins}_{ST}` = 40 — a 3-digit spin count would exceed and is blocked by the guards. Verified per the template-literal limitation: SUO/RAF/LC-REACT rendered through a real JS engine, 26/26 executed checks incl. a byte-match of the stakeholder's example, all three suffix modes for LC-REACT (state / generic US / ON→CA+CAD), Terms lookups unaffected, and negatives (suffix-without-amount, v2.x shapes) rejected; all other builders hash-identical to v3.0.
- **v3.1 — LC-CHURN-DM regions restricted to US / CA.** State/province options are removed from the Churn region dropdown (`updateJurisdictionOptions`, keep-if-valid default US) — Churn's freeforms plus a state suffix would exceed the 40-character NATS limit (e.g. `..._M50_50M200_ON` = 42). Format and parser unchanged.
- **v3.1 — Drop mechanic stays `DR`.** A proposed rename to `DROP` was reverted: `DROP` plus the state suffix would push SUO DAY10 codes to 41 characters.
...
**CA offers (v3.4):** twelve defaults shown unselected: $100/50%/$250, $100/50%/$5,000, $100/100%/$250, $250/10%/$250, $250/10%/$2,500, $250/10%/$5,000, $250/20%/$500, $250/20%/$1,000, $250/20%/$2,000, $250/20%/$5,000, $250/20%/$10,000, $250/20%/$20,000 (Terms IDs `CAS_CA_0078`–`CAS_CA_0086` plus `CAS_CA_0092`–`CAS_CA_0094`; the three v3.3/v3.4 additions carry a baked 09/01/26 Terms start-date override — see the VIPDM T&C Data section).
...
Offer key = `M{min}_{pct}M{max}`. Filings 0078–0086 share the region promotional period, default **August 17, 2026 → December 31, 2026** (editable per the Terms modal; dates render as `{start_date_long}` / `{end_date_long}` in §2 of the T&C). The three v3.3/v3.4 tiers (0092–0094, marked ⚑) were filed **September 1, 2026 → December 31, 2026** and carry a baked start-date override via `VIPDM_TERMS_START_OVERRIDES` → the generated script's `TERMS_START_OVERRIDES` map, consulted by `format_hiw`/`format_tc` per tier key before falling back to the shared `TERMS_START_SHORT/LONG` — the modal's start-date field does not affect them (a warning in the modal names them). The shared end date applies to all twelve.

| Offer key | Min | % | Max CC | Terms ID |
|---|---|---|---|---|
| `M100_50M250` | $100 | 50 | $250 | `CAS_CA_0078` |
| `M100_50M5000` ⚑ | $100 | 50 | $5,000 | `CAS_CA_0094` |
...
| `M250_20M10000` ⚑ | $250 | 20 | $10,000 | `CAS_CA_0092` |
| `M250_20M20000` ⚑ | $250 | 20 | $20,000 | `CAS_CA_0093` |
...
| Default tiers | 12 (`CAS_CA_0078`–`0086` + `CAS_CA_0092`–`0094`, v3.3/v3.4) | 10 (`CAS_10677`–`10686`; adds 10%/$500 and 10%/$1,000, drops 10%/$250) |
| Promotional dates (defaults) | `08/17/26 → 12/31/26` (0092–0094 override start to `09/01/26`) | `07/01/26 → 12/31/26` |
...
**CA image filenames:** `Promo Detail.png`, `Masthead.png`, `{pct}M{max}.png`. Min deposit is not part of the Discover filename. Full default-tier set (v3.4): `50M250.png`, `50M5000.png`, `100M250.png`, `10M250.png`, `10M2500.png`, `10M5000.png`, `20M500.png`, `20M1000.png`, `20M2000.png`, `20M5000.png`, `20M10000.png`, `20M20000.png`.
...
> ⚠️ Several filenames differ by one character — CA: `10M250.png` vs `100M250.png`, and (v3.3) `20M1000.png` vs `20M10000.png`, `20M2000.png` vs `20M20000.png`; US: `10M500.png` vs `10M5000.png`, `20M500.png` vs `20M5000.png`, and `100M250.png` vs `10M250`-style shapes. Double-check the creatives land in the correct files; a mislabeled file uploads the wrong tier's image without erroring.
...
| Jurisdiction at code position 1 (v3.1: always the group US or CA) | Currency row |
|---|---|
| `US`, `MI`, `WV`, `PA`, `NJ` | `USD` |
| `ON`, `AB`, `CA` | `CAD` |
...
| 5 | VIPDM | HIW, T&C, Terms IDs, and promotional dates are baked in at generation time — changes via the Edit T&Cs / Terms modal require regenerating the script. New offer sizes require a Terms ID (editable in the Terms modal); active offers without one block script generation by design. As of v3.3, CA tiers filed with a start date differing from the shared region default get an entry in `VIPDM_TERMS_START_OVERRIDES` in the HTML (currently `M250_20M10000` / `M250_20M20000` / `M100_50M5000` → 09/01/26); the modal's start-date field does not affect overridden tiers. The CAS_CA_0092–0094 filings were supplied and byte-verified. |
...
| v3-2 | VIPDM / VIPBG | K-notation is applied at generation time (values ≥1000 divisible by 1000). Exact-40 codes exist (`M250_10M2500`, `TBG…B100K_G2500`) — any future tier addition must re-check the budget; the generation-time guard blocks silent overflow. The v3.3/v3.4 CA tiers land at 39/39/38 (`M250_20M10K` / `M250_20M20K` / `M100_50M5K`). |
