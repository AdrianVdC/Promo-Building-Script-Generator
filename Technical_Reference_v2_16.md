# NATS Bonus Creator — Technical Reference v2.16

## Changes from v2.15

- **Lifecycle - REACT CC Drop** — eighth offer type fully integrated into the HTML tool. Builds Segment, Promotion, and Bonus for each amount on each day card. Generates `lc_react_MMDDYY_HHMM.py`. Segment dropdown click fixed to use anchored regex scoped to visible dropdown (consistent with Promotion Tile selector pattern already used in all other offer types).
...
**Edit HIW / Edit T&Cs**
Hidden for RAF, SUO, and BG-RE (copy pending legal approval or not applicable). Active for RTC CC, RTC FS, BG, DM, and LC-REACT.

**Edit Images**
Present for RTC CC, RTC FS, RAF, and LC-REACT. Hidden for BG, DM, SUO, and BG-RE.

**+ Add Day**
No-op for RAF and SUO. Max 8 days for BG/DM, 99 for RTC CC/FS/LC-REACT.

**Clear Saved Settings**
Clears all localStorage keys including `lc_react_image_path`.
...
## Per-Code Step Toggles (RTC CC / FS / BG / DM / LC-REACT)
...
Applies to all seven NATS offer types (RTC CC, RTC FS, BG, DM, RAF, SUO, LC-REACT). BG-RE bonuses are built in Playmaker, not NATS, so platform tags do not apply.
...
- Google Drive for Desktop required for RTC CC, RTC FS, RAF, and LC-REACT image path access
