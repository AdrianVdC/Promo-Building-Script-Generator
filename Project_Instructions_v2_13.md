...
**Current live version:** `nats_bonus_creator_v2_13.html`
...
## Offer Types
Five offer types are supported: **RTC Top Up - Casino Credit**, **RTC Top Up - Free Spins**, **Bet & Get (BG)**, **Deposit Match (DM)**, **Refer a Friend (RAF)**.
...
- RTC CC / BG / DM: US (default), MI, WV, PA, NJ, ON, AB, CA
- RTC FS / RAF: MI, NJ, WV, PA only
...
Each nav step moves the mouse to a safe zone (600, 400), hovers the sidebar icon, then uses `dispatch_event("click")` on the submenu item. Up to 5 retry attempts with safe-zone reset. Applies to all five offer types including RAF.
...
This applies to all five offer types.
...
> ⚠️ This differs from all other offer types (BG, DM, RTC FS, RAF), which use a midnight-to-midnight (00:00:00 ET) window.
...
All five `create_promo` functions poll the "Create Promotion" button up to 60 seconds (120 × 500ms) before clicking, to allow the UI time to re-enable it after returning from a previous promo save. Matches the existing Create Bonus poll pattern.
...
Uses regex anchored exact match scoped to `.ant-select-dropdown:not(.ant-select-dropdown-hidden)`. Applied to all offer types.
...
### Image Upload Timing
After file selection, scripts wait **8 seconds** before clicking OK, then 2 seconds after OK. Each image upload takes ~10 seconds total; 3 images per promo = ~30 seconds per promo build. Applies to all five offer types.
...
|---|---|---|
| Generated by `nats_bonus_creator_v2_13.html` | Full end-to-end campaign via HTML tool | ✅ Production |
| `raf_full_campaign_v1_16_final.py` | Standalone script with same logic | ✅ Production |

**Standalone script constants:**

| Constant | Description |
|---|---|
| `DATE` | 6-digit date (MMDDYY) |
| `JURISDICTION` | MI, NJ, WV, or PA |
| `SPIN_AMOUNT` | Per-day spin count |
| `NUM_DAYS` | Total campaign days — **must be 5 or 10** |
| `BET_AMOUNT` | Qualifying wager amount |
| `SPIN_VALUE` | Dollar value per spin (e.g. `"0.10"`) |
| `GAME_KEY` | TCE, 7FH, 7P5, or WWE |

**Run command (Mac):**
```
cd ~/Downloads && python3 raf_full_campaign_MMDDYY_HHMM.py
```
