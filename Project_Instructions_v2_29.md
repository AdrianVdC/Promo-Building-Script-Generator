...
> ⚠️ **Bet & Get - Rules Engine is fully integrated as of v2.15 but currently uses test environment URLs.** NATS and Playmaker links must be switched to production once the Rewards Engine is live in the Playmaker production environment. **Daily Missions - Canada (v2.28) uses production URLs for both NATS and Playmaker** and is the first offer type to build bonuses in production Playmaker. **DMCA Free Spins days (Mon/Wed/Fri/Sun) are locked** — Playmaker Bonus Spins setup is not yet working; only Casino Credit days (Tue/Thu/Sat) can be generated. Full selector reference and field spec are in `Technical_Reference_v2_29.md` attached to this project.
...
The typed date's weekday determines everything — reward type, tiers, Terms IDs, game, images, and title. There are no reward-type, game, or day dropdowns; mismatches are impossible by construction.

| Weekday | Reward | Game | Terms IDs (tier order) |
|---|---|---|---|
| Tuesday / Thursday / Saturday | **Casino Credit** (shared set) | — | `CAS_CA_0019`–`0022` |
| Monday | Free Spins | Baa Baa Baa™ (BAA) | `CAS_CA_0043`–`0046` |
| Wednesday | Free Spins | 4 Crazy Cluckers™ (4CC) | `CAS_CA_0047`–`0050` |
| Friday | Free Spins | Mega Fire Blaze™: Legacy of the Tiger™ (MFB) | `CAS_CA_0051`–`0054` |
| Sunday | Free Spins | Blue Wizard: Cash Collect & Link™ (BWZ) | `CAS_CA_0055`–`0058` |

> ⚠️ **Free Spins days are LOCKED as of v2.28** — Playmaker Bonus Spins setup is not yet working. FS-day cards render an amber lock notice with the four FS tiers greyed and disabled (showing that weekday's Terms IDs), and script generation is blocked with an alert if any card has an FS-day date. Only Casino Credit days (Tue/Thu/Sat) generate. FS tiers, per-day Terms IDs, the weekday→game map, and the FS toast copy are all pre-configured in the HTML for the unlock; the Playmaker FS field spec is recorded in the Technical Reference.
...
- **FS-day state:** amber lock notice; greyed disabled tiers with that weekday's Terms IDs.
- **Step badges: Promo + Bonus only** (BG-RE style — the segment is attached inside Playmaker by code name; no NATS segment).
- Edit HIW / Edit T&Cs buttons hidden (copy baked in — changes require reaching out to Adrian). Edit Images visible.

### Naming & Codes
- **CC code:** `MMDDYY_CAS_RET_BG_CA_CC_B{bet}_G{get}` (e.g. `120126_CAS_RET_BG_CA_CC_B125_G5`) — used identically for the NATS promo name, Playmaker bonus name, and Playmaker segment chip. ⚠️ Shape-identical to a standard BG CA code by stakeholder acceptance — no distinguishing token; avoid building a standard BG CA offer with the same date/bet/get.
- **FS code (reserved, locked):** `MMDDYY_CAS_RET_BG_CA_FS_B{bet}_G{spins}S_{value}V_{game}` with the spin value in cents (e.g. `113026_CAS_RET_BG_CA_FS_B125_G50S_10V_BAA`).
...
- **Playmaker bonus:** 00:00 ET typed date → 00:00 ET next day (BG-RE shape).

### NATS Promo (Phase 1)
Image only CTA / Overlay. **Title and Promo Header Text auto-derived: `{Weekday}'s Daily Mission`** (e.g. `Tuesday's Daily Mission`). **Bonus Tile ON. Button 2 empty.** No confirmation modal. Images from the local Drive folder with **underscored, weekday-prefixed filenames** (see Image System). A **pre-flight check** verifies every required image file on disk **before login** and aborts with a missing-file list.

### Playmaker Bonus (Phase 2 — Save Bonus IS clicked)
Steps 1–6: **Canada** jurisdiction (`#jurisdiction-ontario`), start/end dates via the calendar widget with both times forced to 00:00, **segment chip = code name** (fill + Enter — chip attach confirmed in PROD), wagering amount = bet, **Casino Credit** reward radio, settlement amount = get, Days to Use Reward from the card, promotion selected by code name, **toast always ON** (`Promotion Complete!` / `You have been awarded $X Casino Credit`; FS variant `You have been awarded {spins} Bonus Spins to "{game_name}"` recorded for the unlock), then **Save Bonus**.
...
**HIW (CC; no "CAD" by design; "twenty-four (24) hours" static legal copy):**
```
1) Opt in
...
```
**FS HIW (recorded for the unlock):** line 3 becomes `We'll reward you with {spins} Bonus Spins to use on the slots game {game_name}`.

**CC T&C:** full Ontario legal copy (FBG Enterprises Canada, Inc., 19+/Ontario, ConnexOntario), one template for all four CC tiers. Dynamic fields: `{bet_fmt}` (comma-formatted), `{get_fmt}`, `{terms_id}`, `{start_date_long}`, `{end_date_long}`. Title line: `FANATICS CASINO (CANADA) – BET {bet_fmt} CAD, GET {get_fmt} CAD IN CASINO CREDIT ({terms_id})`. **Static by design:** §5's $2/$20/$5 worked examples on all tiers, the CC-only "each time you are eligible" clauses in §4/§7, the $15-winnings-style examples, and the seven (7) day expiry. Byte-verified against the supplied CAS_CA_0019/0020 filings with two approved normalizations: (1) `{game_name}` always followed by a space in the FS template (the Monday filings' glued "Baa Baa Baa™and" defect), (2) trailing whitespace stripped. Promotional window **08/17/26 → 12/31/26** baked at generation time — five independent date pairs exist in the spec (Mon/Wed/Fri/Sun/CC-shared; FS windows will change when games rotate), with the CC pair live in v2.28.

**FS T&C:** template locked in the spec (dynamic: `{bet_fmt}`, `{spins}`, `{spin_value}` two-decimals, `{game_name}`, `{terms_id}`, dates; title `FANATICS CASINO (CANADA) – BET {bet_fmt} CAD, GET {spins} BONUS SPINS ({terms_id})`) but **not yet wired into the builder** — part of the FS unlock.

> ⚠️ CAS_CA_0021/0022 (CC $500/$5,000 tiers) and all FS-day filings were not supplied — the templates are assumed to cover them; verify on first use.

### PROD Validation (Aug 14, 2026)
- NATS promo `120126_CAS_RET_BG_CA_CC_B125_G5` built live end-to-end via the one-time validation script (Tuesday images, single-day window, CAS_CA_0020 T&C verbatim, Bonus Tile ON). ⚠️ **This test promo still exists in PROD NATS** — delete it or avoid reusing that exact code to prevent a duplicate-name collision.
- Playmaker dry run filled Steps 1–6 in production (Canada, CC radio, wagering 125, settlement 5, promo + segment chip attached) with Save Bonus located but not clicked.
- The v2.28 generated script has not yet had its own full end-to-end run (promo + bonus with Save clicked) — the first real generation run should confirm the saved bonus lands correctly.
...
- DMCA (v2.28): **underscored, weekday-prefixed, weekday auto-derived from the typed date** — `{Weekday}_Promo_Detail.png`, `{Weekday}_Masthead.png`, CC discover `{Weekday}_B{bet}_G{get}.png` (e.g. `Tuesday_B125_G5.png`); FS discover reserved as `{Weekday}_B{bet}_G{spins}S_{value}V.png` (no game token — the weekday implies the game)
...
| 14 | DMCA | **Free Spins days (Mon/Wed/Fri/Sun) are locked** — Playmaker Bonus Spins setup is not yet functional; only Casino Credit days (Tue/Thu/Sat) generate. Unlocking requires: Playmaker Bonus Spins working, Aggregator/Provider/Casino Game values per weekday game (not yet known — to be configured later), confirmation that the Free Spin Stakes row offers CAD for Canada (observed USD in the dry run), FS toast copy wiring, and wiring the recorded FS T&C template into the builder. FS tiers, per-day Terms IDs, and the weekday→game map are already configured in the HTML. |
| 15 | DMCA | HIW, T&C, Terms IDs, and promotional dates are baked into the HTML with **no edit modals** (Edit HIW / Edit T&Cs hidden by design) — changes require editing the HTML (reach out to Adrian) and regenerating scripts. The CC T&C was byte-verified against the supplied CAS_CA_0019/0020 filings (with two approved normalizations: `{game_name}` trailing space in the FS template; trailing whitespace stripped); **CAS_CA_0021/0022 and all FS-day filings were not supplied** — verify on first use. |
| 16 | DMCA | CC code names are shape-identical to standard BG CA codes (`MMDDYY_CAS_RET_BG_CA_CC_B{bet}_G{get}`) — accepted by stakeholder decision, no distinguishing token. Avoid building a standard BG CA offer with the same date/bet/get. The Aug 14 2026 validation promo `120126_CAS_RET_BG_CA_CC_B125_G5` still exists in PROD NATS — delete it or avoid reusing that exact code. |
| 17 | DMCA | The Okta Verify (FastPass) persistent-profile launch (`~/.dm_canada_pw_profile` + `--disable-features=LocalNetworkAccessChecks` + `local-network-access` grant) is DMCA-only; all other offer types still use plain `chromium.launch()`. Back-porting is a candidate follow-up if Okta blocks appear elsewhere. The v2.28 generated script has not yet had a full end-to-end run with Save Bonus clicked — verify the first live run. |
...
**Image filenames (underscored, weekday-prefixed — weekday auto-derived from the typed date):** `{Weekday}_Promo_Detail.png`, `{Weekday}_Masthead.png`, CC discover `{Weekday}_B{bet}_G{get}.png` (e.g. `Tuesday_Promo_Detail.png`, `Tuesday_B125_G5.png`). FS discover reserved as `{Weekday}_B{bet}_G{spins}S_{value}V.png` (no game token — the weekday implies the game). Each usable weekday needs its own Promo Detail + Masthead + one discover per selected tier. The generated script **pre-flight checks every required file before login** and aborts with a missing-file list if any are absent.
...
Each nav step moves the mouse to a safe zone (600, 400), hovers the sidebar icon, then uses `dispatch_event("click")` on the submenu item. Up to 5 retry attempts with safe-zone reset. Applies to all offer types that use NATS. DMCA's NATS phase uses only the Promos nav (its bonus lives in Playmaker).
...
### Bet & Get - Rules Engine / Daily Missions - Canada
**Promo** and **Bonus** badges only — no Segment badge. The segment is created/attached inside Playmaker as part of the bonus flow (DMCA attaches it as a chip by code name in Step 2).
...
BG-RE and DMCA bonuses are built in Playmaker, not NATS, so platform tags do not apply.
...
> ⚠️ **NATS game option names include trademark symbols as of August 2026** (e.g. `Triple Cash Eruption™ (WV)`). As of v2.20/v2.21, scripts type only the pre-™ portion of the Search Name into the Casino Game search box, then select the option with an anchored regex where every `™` is optional but the jurisdiction suffix is required. The Search Name column above does **not** need editing when NATS adds/removes `™`. Applies to RTC FS, RAF, and SUO. (The DMCA weekday games are a separate map used only for Playmaker FS setup, which is not yet live.)
...
Fully integrated into the HTML tool as of v2.15. See `Technical_Reference_v2_29.md` for full selector reference and Playmaker field spec.
...
LC-REACT, LC-CHURN-DM, VIPDM, and VIPBG use the same anchored regex pattern as the Promotion Tile — scoped to `.ant-select-dropdown:not(.ant-select-dropdown-hidden)` — to avoid hitting the hidden search mirror span that NATS injects alongside the input. DMCA has no NATS segment — its segment is attached in Playmaker as a chip (fill `input[cmdk-input]` with the code name + Enter).
...
### DMCA Single-Day Promo Window & Weekday Images (v2.28)
DMCA's `parse_dates(code)` returns the typed date `00:00:00 ET → 23:59:59 ET` — the only single-day window in the tool. The weekday is derived at runtime from the code's date and drives the promo Title/Header (`{Weekday}'s Daily Mission`) and all three image filenames (`{Weekday}_Promo_Detail.png` / `{Weekday}_Masthead.png` / `{Weekday}_B{bet}_G{get}.png`). A pre-flight loop verifies every required file on disk before login and aborts with a missing-file list.
...
```
**FS HIW (recorded for the unlock):** line 3 becomes `We'll reward you with {spins} Bonus Spins to use on the slots game {game_name}`.
**CC T&C:** Full Canadian (Ontario) legal copy — one template for all four CC tiers, byte-verified against the supplied CAS_CA_0019/0020 filings. Dynamic fields: `{bet_fmt}` (comma-formatted), `{get_fmt}`, `{terms_id}`, `{start_date_long}`, `{end_date_long}`. Title line: `FANATICS CASINO (CANADA) – BET {bet_fmt} CAD, GET {get_fmt} CAD IN CASINO CREDIT ({terms_id})`. **Static by design:** §5's $2/$20/$5 worked examples on all tiers, the CC-only "each time you are eligible" clauses in §4/§7, and the seven (7) day expiry — do not template them. Promotional window (currently August 17, 2026 → December 31, 2026) is baked at generation time; the spec defines five independent date pairs (Mon/Wed/Fri/Sun/CC-shared — FS windows change when games rotate), with the CC pair live in v2.28.
**FS T&C (recorded, not yet in the builder):** dynamic fields `{bet_fmt}`, `{spins}`, `{spin_value}` (two decimals), `{game_name}`, `{terms_id}`, dates; title `FANATICS CASINO (CANADA) – BET {bet_fmt} CAD, GET {spins} BONUS SPINS ({terms_id})`; approved normalization: `{game_name}` always followed by a space (the Monday filings' glued "Baa Baa Baa™and" defect). ⚠️ CAS_CA_0021/0022 and all FS-day filings were not supplied — verify on first use.
